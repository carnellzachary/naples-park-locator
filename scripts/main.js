...
let naplesMap = L.map('naplesMap').setView([26.193813, -81.705949], 12.33);
...
for (let i = 0; i < places.length; i++) {
  ...
  places[i].marker.addTo(naplesMap);
}
...
function markerOnClick() {
  ...
  // Transition header to show description
  document.querySelector('#mainContainer').classList = 'mainContainerShowDesc';
...
function repopulateMap() {
  ...
  for (let i = 0; i < parksByCriteria.length; i++) {
    ...
    parksByCriteria[i].marker.addTo(naplesMap);
  }
}
